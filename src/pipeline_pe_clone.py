...
class FluxPipeline(DiffusionPipeline, FluxLoraLoaderMixin, FromSingleFileMixin):
    ...
    def __call__(
        ...
        if isinstance(prompt_embeds,torch.Tensor) and prompt==None:
            text_ids=torch.zeros(prompt_embeds.shape[1], 3).to(device=device, dtype=self.transformer.dtype)
        else:
            (
                prompt_embeds,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #print(latents.shape,pooled_prompt_embeds.shape,prompt_embeds.shape)#[1, 3072, 64]) torch.Size([1, 768]) torch.Size([1, 256, 4096])
